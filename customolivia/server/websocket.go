...
type ResponseMessage struct {
	...
	Tag         string           `json:"tag"`
	Information user.Information `json:"information"`
	Action      *start.AppAction `json:"action"`
}
...
func Reply(request RequestMessage) []byte {
	...
	if IsJSON(responseSentence) {
		var customMessageModule start.CustomModuleResponse
		err := json.Unmarshal([]byte(responseSentence), &customMessageModule)
		if err != nil {
			response = ResponseMessage{
			...
			response = ResponseMessage{
				Content:     customMessageModule.Content,
				Tag:         responseTag,
				Information: user.GetUserInformation(request.Token),
				Action:      &customMessageModule.Action,
			}
		}
