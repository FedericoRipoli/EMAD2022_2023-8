...
class UtenteService{
  ...
  Future<List<Utente>?> usersList(String? username, String? ente, bool? admin, int page) async{
    String? token=await userService.getUser();
    try{
      var response = await http.get(
          RestURL.utenteService,
          headers: RestURL.authHeader(token!)
      );
      if (response.statusCode == 200) {
        ListResponse<Utente> l=ListResponse<Utente>.fromJson(jsonDecode(response.body), Utente.fromJson);
        return l.content;
      }
    }
    catch(e){
